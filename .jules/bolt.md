...
## 2025-05-28 - Optimize String Replacement in Shell Scripts
**Learning:** Using `python3 -c` for simple file string replacements (like placeholder substitution) in bash scripts introduces unnecessary Python interpreter startup overhead.
**Action:** Always prefer native Unix tools like `sed` (e.g., `sed -i "s/old/new/g" file`) for simple text replacements in bash scripts to maximize performance.
