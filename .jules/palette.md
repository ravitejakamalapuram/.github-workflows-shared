...
## 2026-06-07 - Implement aria-disabled for disabled button tooltips
**Learning:** Replacing native `disabled` with `aria-disabled="true"` to allow tooltips for screen readers and hovers natively exposes elements to click events. Thus, you must implement javascript event guards on the handler directly. Furthermore, when removing native disabled, you should manage the visual disabled state using CSS attribute selectors (e.g. `[aria-disabled="true"]`) to ensure the element looks inactive.
**Action:** When replacing `disabled` with `aria-disabled='true'`, always ensure click events are blocked using `if(this.getAttribute('aria-disabled') === 'true') return;` or equivalent guard logic to prevent functional regressions, and use attribute CSS targeting for visual feedback.
