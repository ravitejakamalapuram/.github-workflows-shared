...
<body>
            ...
            <div class="workspace-layout" style="grid-template-columns: 1fr 400px;">
                ...
                <div class="panel">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="package-build-script" placeholder="e.g. npm run build" oninput="const btn = document.getElementById('btn-run-build'); const isDisabled = !this.value.trim(); btn.setAttribute('aria-disabled', isDisabled ? 'true' : 'false'); isDisabled ? btn.setAttribute('title', 'Enter a build script command to enable') : btn.removeAttribute('title');">
                    </div>
                    ...
                    <div style="background: rgba(30, 41, 59, 0.2); border: 1px solid var(--card-border); border-radius: 10px; padding: 16px; display: flex; align-items: center; justify-content: space-between; gap:12px; flex-wrap: wrap; margin-top: 8px;">
                        ...
                        <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                            ...
                            <button class="btn btn-primary" id="btn-run-build" onclick="triggerBuild()" title="Enter a build script command to enable">Execute Build</button>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function setupBuildExecutorPanel() {
            ...
            if (state.selectedRepo.metadata_exists && state.selectedRepo.metadata.modules) {
                ...
                if (mod) {
                    ...
                    const isDisabled = !document.getElementById("package-build-script").value.trim();
                    btn.setAttribute("aria-disabled", isDisabled ? "true" : "false");
                    btn.removeAttribute("disabled");
                    isDisabled ? btn.setAttribute("title", "Enter a build script command to enable") : btn.removeAttribute("title");
                }
            } else {
                nameEl.innerText = `${state.selectedRepo.appName}`;
                btn.setAttribute("aria-disabled", "true");
                btn.removeAttribute("disabled");
                btn.setAttribute("title", "Enter a build script command to enable");
            }
        }
        ...
        function triggerBuild() {
            const btn = document.getElementById("btn-run-build");
            if (btn.getAttribute("aria-disabled") === "true" && document.querySelector(".terminal-panel.active-build")) return;
